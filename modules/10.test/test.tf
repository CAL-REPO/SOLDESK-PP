...
resource "aws_default_security_group" "SG_DEFAULT" {
    count = (length(aws_vpc.VPC) > 0 &&
            var.VPC["SG_DEFAULT_NAME"] != "" ? 1 : 0)

    vpc_id = aws_vpc.VPC[count.index].id

    tags = {
        Name = var.VPC["SG_DEFAULT_NAME"]
    }
}
...
resource "aws_vpc_dhcp_options" "DHCP"{
    ...
    domain_name = var.VPC["DOMAIN"]
    domain_name_servers = var.VPC["DOMAIN_NS"]
    ntp_servers  = var.VPC["DOMAIN_NTP"]
}
...
resource "aws_route_table" "RTB" {
    count = (length(var.VPC_RTB) > 0 ?
            length(var.VPC_RTB) : 0)
    depends_on = [aws_vpc.VPC]
    vpc_id = aws_vpc.VPC[0].id
