...
resource "aws_route53_zone" "ZONE" {
    count = (length(var.ZONE) > 0 ? length(var.ZONE) : 0 )

    name         = var.ZONE[count.index].DOMAIN_NAME
    tags = {
        Name = "${var.ZONE[count.index].NAME}"
    }
    vpc {
        vpc_region = "${var.ZONE[count.index].TYPE_PRIVATE}" == true ? var.ZONE[count.index].REGION_ID : null
        vpc_id     = "${var.ZONE[count.index].TYPE_PRIVATE}" == true ? var.ZONE[count.index].VPC_ID : null  
    }
}

resource "aws_route53_record" "ZONE_RECORD" {
    count = (length(var.ZONE_RECORD) > 0 ? length(var.ZONE_RECORD) : 0 )

    zone_id = var.ZONE_RECORD[count.index].ZONE_ID
    name    = var.ZONE_RECORD[count.index].NAME
    type    = var.ZONE_RECORD[count.index].TYPE
    ttl     = var.ZONE_RECORD[count.index].TTL
    records = var.ZONE_RECORD[count.index].IPS
}

resource "aws_route53_resolver_endpoint" "RESOLV_EP" {
    count = (length(var.RESOLV_EP) > 0 ? length(var.RESOLV_EP) : 0 )
    name      = var.RESOLV_EP[count.index].NAME
    direction = var.RESOLV_EP[count.index].DIRECTION

    security_group_ids = var.RESOLV_EP[count.index].SG_IDS

    dynamic "ip_address" {
        for_each = var.RESOLV_EP[count.index].IPS
        content {
...
resource "aws_route53_resolver_rule" "RESOLV_EP_RULE" {
    count = (length(var.RESOLV_EP_RULE) > 0 ? length(var.RESOLV_EP_RULE) : 0 )

    name        = var.RESOLV_EP_RULE[count.index].NAME
    resolver_endpoint_id = var.RESOLV_EP_RULE[count.index].EP_ID
    domain_name = var.RESOLV_EP_RULE[count.index].DOMAIN_NAME
    rule_type = var.RESOLV_EP_RULE[count.index].RULE_TYPE

    dynamic "target_ip" {
        for_each = var.RESOLV_EP_RULE[count.index].IPS
        content {
...
resource "aws_route53_resolver_rule_association" "RESOLV_EP_RULE_ASS" {
    count = (length(var.RESOLV_EP_RULE) > 0 ? length(var.RESOLV_EP_RULE) : 0 )

    resolver_rule_id = aws_route53_resolver_rule.RESOLV_EP_RULE[count.index].id
    vpc_id           = var.RESOLV_EP_RULE[count.index].VPC_ID
}
...
resource "aws_lb_target_group" "LB_TG" {
    ...
    health_check {
        ...
        protocol = try(var.LB_TG[count.index].PROTOCOL, "HTTP")
        port = try(var.LB_TG[count.index].PORT, "Traffic-port")
        path = try(var.LB_TG[count.index].PATH, "/") 
        healthy_threshold = try(var.LB_TG[count.index].HEALTHY_THRESHOLD, 5)
        unhealthy_threshold = try(var.LB_TG[count.index].UNHEALTY_THRESHOLD, 5)
        timeout = try(var.LB_TG[count.index].TIMEOUT, 5)
        interval = try(var.LB_TG[count.index].INTERVAL, 30)
        matcher = try(var.LB_TG[count.index].MATCHER, 200)
    }
